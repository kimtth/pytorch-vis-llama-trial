...
def parse_args():
    ...
    # @TODO
    llm_hf_path = os.path.join(os.getcwd(), 'model', 'Llama-2-7b-hf')
    train_file_path = os.path.join(os.getcwd(), 'vsllm', 'train.pkl')
    img_length = '10'

    # Load pickle file then print out inside of it
    import pickle
    with open(train_file_path, 'rb') as f:
        data = pickle.load(f)
        print(data)
        img_length = str(len(data['text']))

    args = parser.parse_args(['--train_file', train_file_path, '--model_name_or_path', llm_hf_path, '--image_length', img_length])
...
def main():
    ...
    CUDA out of memory. Tried to allocate 172.00 MiB. GPU 0 has a total capacty of 6.00 GiB of which 0 bytes is free. 
    Of the allocated memory 20.02 GiB is allocated by PyTorch, and 19.17 MiB is reserved by PyTorch but unallocated.
    ...
    import gc
    torch.cuda.empty_cache() # Exception has occurred: OutOfMemoryError
    gc.collect()
    # del variables
    ...
    for epoch in range(args.num_train_epochs): 
        ...
        if is_master(args):
            print(f'{os.linesep} save modeling')
